...
import bookLogo from "../../asset/Crown.jpg"
...
class CartItems extends Component {
    ...
    remove = (id) => {
        CartService.remove(id).then(response => {
    ...
    updateCount = (id, count) => {
        ...
        CartService.updateCart(cartDTO.cartId,cartDTO.quantity,cartDTO.totalPrice).then(response => {
            this.props.handleCart()
        }).catch((error) => {
            console.log(error)
        })
    ...
    render() {
        return (
            <div className="mycart">
                <div>
                    <img src={this.props.books.bookLogo} alt="Not found" className="mycart-img"/>
                </div>
                <div className="books-container">
                    <Typography component="h2" id="bookname1">{this.props.books.bookName}</Typography>
                    <Typography variant="body2" color="textSecondary" id="authorName">{this.props.books.bookAuthor}</Typography>
